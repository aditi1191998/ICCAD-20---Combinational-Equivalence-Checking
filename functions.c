...
#include "keywords.h"
#include "functions.h"
node_t * final_exp[9999];

void initialize(unsigned int **sorted,int index,int order,circuit c,int size)
{
	node_t * pInt = NULL; 
    wire w;
for (int i = index+1; i < order; i++) {
    ...
    w = getWire((*sorted)[i],c);
     deleteList(final_exp[w->outputs[0]]);
         final_exp[w->outputs[0]] = pInt;
    }
}
}
/**
...
for (int i=0;i<9999;i++)
{
 ...
 /*  FILE* write_file;
   ...
   for ( int o = 0 ; o < m->outputcount ; o++ )
    {
       // fputs("Number of primary inputs:", write_file[o]);
   sprintf(str, "%d", m->inputcount);
   fputs(str,write_file[o]);
   fputs(" ",write_file[o]);
   //fputs("Number of primary outputs:", write_file[o]);
...
void dc_case(module m, FILE ** fptr,unsigned int **sorted,circuit c,int order,int index)
{
    ...
    //printf("debug1\n");
    int n = m->inputcount + m->outputcount + m->wirecount + m->gatecount;
    initialize(&(*sorted),index,order,c,(n-(m->gatecount)));
    for (int i = index+1; i < order; i++) {
...
for(int a=m->inputcount; a<(m->inputcount+m->outputcount);a++)
    {
        ...
        char str[10]; 
        sprintf(str, "%d", a); 
        fputs("O",fptr[a-m->inputcount]);
        fputs(str,fptr[a-m->inputcount]);
        fputs("=",fptr[a-m->inputcount]);
        list_print(final_exp[a],fptr[a-m->inputcount]);
        fputs("\n",fptr[a-m->inputcount]);
        printf("\n");
    }
  //  putchar('\n');
...
// Part 3 - the rest of the list
    ...
    else if(!strcmp(w->type,"^") || !strcmp(w->type,"~^") )
        {
            ...
            {
                node_t *newHead = malloc(sizeof(node_t));
                node_t *current = final_exp[w->inputs[0]] ;
                node_t *current_out = final_exp[w->outputs[0]];
                strcpy(newHead->s, current->s);
                newHead->next = NULL;
                while(current_out->next != NULL) {
                       current_out = current_out->next;
...
void list_print(node_t * head, FILE* fptr) {
    ...
    while (current != NULL) {
      printf("%s ", current->s);
        if(!strcmp(current->s,"(") || !strcmp(current->s,"))")  || !strcmp(current->s,")") || !strcmp(current->s,"&") || !strcmp(current->s,"~&") || !strcmp(current->s,"|") || !strcmp(current->s,"~|") || !strcmp(current->s,"buf") || !strcmp(current->s,"^") || !strcmp(current->s,"~^") || !strcmp(current->s,"~") || !strcmp(current->s," "))
        fputs(current->s,fptr);
        else
        {
           fputs("I",fptr);
           int num = atoi(current->s);
           num = num + 1;
           char s[10];
           sprintf(s,"%d",num);
           fputs(s,fptr);
